...
def generate_html(semgrep_findings, gitleaks_findings, trivy_cves,
    ...
    # ----------------------------------------------------------------
    # ZAP rows — three-way logic
    #   zap_findings   → alerts found, render table rows
    #   zap_was_run    → ZAP ran but 0 alerts (e.g. API-only app)
    #   neither        → DAST was not enabled in the workflow
    # ----------------------------------------------------------------
    if zap_findings:
        zap_rows = ""
        ...
        zap_rows = (
            ...
            '✅ DAST scan completed — ZAP found 0 alerts on this application. '
            'This is expected for API-only apps or applications without HTML pages, '
            'as ZAP baseline scan relies on crawling HTTP responses.'
            '</td></tr>'
        )
    else:
        zap_rows = (
            '<tr><td colspan="5" class="text-center text-muted">'
            'DAST not enabled. Add <code>enable-dast: true</code>, '
            '<code>app-start-command</code>, and <code>app-port</code> '
            'to your workflow to activate OWASP ZAP dynamic scanning.'
            '</td></tr>'
        )

    # ----------------------------------------------------------------
    # ZAP metric card description — three-way logic
    # ----------------------------------------------------------------
    if zap_findings:
        zap_desc = (
            "<strong style='color:#a371f7'>DAST scan completed.</strong> "
            "ZAP performed a baseline scan against the running application via HTTP. "
            "Dynamic analysis complements static tools by detecting runtime issues "
            "such as missing security headers and CORS misconfigurations."
        )
    elif zap_was_run:
        zap_desc = (
            "<strong style='color:#3fb950'>DAST scan completed — 0 alerts detected.</strong> "
            "ZAP performed a baseline scan but found no issues on this application. "
            "This is expected behaviour for API-only applications or apps that return "
            "primarily JSON responses, as ZAP's baseline scan relies on crawling HTML pages."
        )
    else:
        zap_desc = (
            "<strong style='color:#8b949e'>DAST not enabled.</strong> "
            "Add <code>enable-dast: true</code>, <code>app-start-command</code>, "
            "and <code>app-port</code> to your workflow to activate OWASP ZAP dynamic scanning."
        )
  ...
  <div class="row g-3 mb-4">
    ...
    <div class="col-md-9">
      <div class="metric-card" style="text-align:left;">
        <small class="text-muted">{zap_desc}</small>
      </div>
    </div>
  </div>"""
...
def main():
    ...
    # Load ZAP separately so we can distinguish:
    #   zap_was_run = True  → file existed (ZAP ran, even if 0 alerts)
    #   zap_was_run = False → file missing (DAST was not enabled)
    _zap_raw   = load_json(zap_path)
    zap_was_run = _zap_raw is not None
    zap_data    = _zap_raw or {}
    ...
    print(f"ZAP was run:       {zap_was_run}")
